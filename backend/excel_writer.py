...
def write_pl(wb, pl_data):
    ...
    # 合算が必要な項目
    ws4["AI19"] = (pl_data.get("staff_salary", 0) + pl_data.get("misc_salary", 0) + pl_data.get("bonuses", 0)) // 1000
    ws4["AI24"] = (pl_data.get("supplies", 0) + pl_data.get("office_supplies", 0)) // 1000
    ws4["AI25"] = (pl_data.get("travel_expense", 0) + pl_data.get("comm_expense", 0)) // 1000
    ...
    # 販管費合計を計算してセットする
    sga_fields = [
        "director_salary", "staff_salary", "misc_salary", "bonuses",
        "welfare_expense", "outsourcing_expense", "travel_expense", "comm_expense",
        "ent_expense", "meeting_expense", "depreciation", "rent", "lease",
        "insurance", "utilities", "supplies", "taxes", "office_supplies",
        "advertising", "fees", "training", "books", "software_expense", "misc_expense",
    ]
    ...
    # 受取利息及び配当金（合算）→ AI4
    ws5["AI4"] = (pl_data.get("interest_income", 0) + pl_data.get("dividend_income", 0)) // 1000

    # 各カテゴリの小計（すでに千円単位のセル値を合算）
    ws5["AV5"]  = (ws5["AI4"].value or 0) + (ws5["AI5"].value or 0)   # 営業外収益合計
    ws5["AV10"] = (ws5["AI7"].value or 0) + (ws5["AI8"].value or 0) + (ws5["AI9"].value or 0) + (ws5["AI10"].value or 0)  # 営業外費用合計
